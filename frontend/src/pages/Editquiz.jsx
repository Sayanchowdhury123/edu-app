...
const Editquiz = () => {
    ...
    const { courseid, lessonid, sectionindex, quizid, quiz } = location.state || {};
    ...
    const addquiz = async () => {
        try {
            const res = await axiosinstance.put(`quiz/${courseid}/sections/${sectionindex}/lessons/${lessonid}/q/${quizid}`, { title: title, que: que, ans: ans, options: options }, {
                headers: {
    ...
    return (
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="p-4">
            ...
            <div className="space-y-4 md:w-md mx-auto">
                ...
                <button className={`btn btn-outline btn-primary btn-sm mb-4 w-full `} onClick={handleoptionadd} disabled={options.length > 4}>Add Option</button>
